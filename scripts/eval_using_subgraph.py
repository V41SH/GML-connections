...
def get_subgraph_edge_set(edge_index: torch.Tensor) -> Set[Tuple[int, int]]:
    ...
    edge_set = set(map(tuple, sorted_edges))

    # Explicitly delete intermediate arrays to free memory
    del edges, sorted_edges
...
class EmbeddingEvaluator:
    ...
    def __init__(self, embedding_path: str, subgraphs_path: str):
        ...
        self.word2idx = None
        self.idx2word = None
        self.all_subgraphs_by_dim = None  # Store raw subgraph data for lazy loading
    ...
    def _load_subgraphs(self):
        """Load the raw DINE dimension subgraphs for lazy processing."""
        ...
        with open(self.subgraphs_path, "rb") as f:
            self.all_subgraphs_by_dim = pickle.load(f)

        print(
            f"Loaded {len(self.all_subgraphs_by_dim)} dimension subgraphs (lazy loading enabled)."
        )

    def _get_edge_set_for_dim(self, dim: int) -> Set[Tuple[int, int]]:
        """Get edge set for a specific dimension (lazy loading)."""
        if dim not in self.all_subgraphs_by_dim:
            return set()

        edge_index = self.all_subgraphs_by_dim[dim]
        edge_set = get_subgraph_edge_set(edge_index)
    ...
    def predict_groups_dine(
        ...
        for group_indices in combinations(valid_words_indices, group_size):
            ...
            for dim in self.all_subgraphs_by_dim.keys():
                edge_set = self._get_edge_set_for_dim(dim)
                if not edge_set:
                    continue
                ...
                # Explicitly delete edge_set to free memory immediately
                del edge_set
    ...
    def evaluate_game(self, game_id: int, csv_path: str) -> Tuple[Dict, List[Dict]]:
        ...
        pred_groups_list, pred_groups_info = self.predict_groups_dine(data["all_words"])
    ...
    def evaluate_multiple_games(self, game_ids: List[int], csv_path: str) -> Dict:
        ...
        all_metrics = []
        best_game = {"id": None, "score": -1, "metrics": None, "info": None}
        ...
        print("  Best Game Predicted Groups & Explanations:")
        print("Ground truth groups:")
        for group in best_game["true_groups"]:
            print(group)

        for info in best_game["info"]:
