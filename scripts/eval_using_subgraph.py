...
class EmbeddingEvaluator:
    ...
    def _load_subgraphs(self):
        ...
        if not os.path.exists(self.subgraphs_path):
            raise FileNotFoundError(f"Subgraph file not found: {self.subgraphs_path}. Run eval_dimensions.py first.")
        ...
        self.dim_edge_sets = {
            ...
            for dim, edge_index in tqdm(all_subgraphs_by_dim.items(), desc="Processing dims")
        }
    ...
    def predict_groups_dine(
        ...
        for group_indices in combinations(valid_words_indices, group_size):
            ...
            score = (best_dim_score + 1e-5) / total_pairs # Add epsilon to prioritize 1-pair over 0-pair
        ...
        for score, group_indices, best_dim in scored_groups:
            if not set(group_indices).intersection(used_indices):
                ...
                final_groups_info.append({
                    ...
                    "dim": best_dim,
                    "score": score,
                    "pairs_found": int(round(score * total_pairs)) # round to remove epsilon
                })
    ...
    def evaluate_multiple_games(self, game_ids: List[int], csv_path: str) -> Dict:
        ...
        for info in best_game['info']:
            ...
            print(f"      -> Explained by: Dim {info['dim']} (Pairs: {info['pairs_found']}/6)")
...
def main():
    ...
    subgraphs_path = os.path.join(base_dir, "dimension_subgraphs_connections_only.pkl")
