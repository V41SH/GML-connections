...
def load_graph_from_gml(gml_path):
    ...
    edge_type = torch.tensor(
        [relation_to_idx[rel] for rel in edge_types], dtype=torch.long
    )

    print(
        f"Graph: {len(nodes)} nodes, {len(edges)} edges, {len(relations)} relation types"
    )
...
def load_fasttext_embeddings(nodes, embedding_dim=300):
    ...
    out_path = os.path.join(
        os.path.dirname(__file__), "../graphs/fasttext_node_embeddings.npy"
    )
...
def compute_dine_loss(
    ...
    # Base loss (BCE)
    base_loss = compute_link_prediction_loss(
        embeddings, pos_edge_index, neg_edge_index, link_predictor
    )
...
def compute_dine_contrastive_loss(
    embeddings,
    pos_edge_index,
    neg_edge_index,
    link_predictor,
    ...
    if loss_fn in ["link_prediction", "dine", "contrastive", "dine_contrastive"]:
        ...
        if loss_fn == "link_prediction":
            ...
            loss = compute_dine_loss(
                ...
                neg_edge_index,
                link_predictor,
                orth_coeff=ortloss_coeff,
                size_coeff=sizeloss_coeff,
            )
