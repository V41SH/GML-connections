...
def segment_compound_words(
    word: str, existing_vocab: Set[str], min_subword_length: int = 3
) -> List[str]:
    """
...
def add_compound_edges(
    ...
    if new_edges:
        ...
        print(
            f"Added {len(new_edges):,} compound edges for {len(new_edges) // 2:,} compound words"
        )
...
def load_conceptnet_graph(csv_path: str) -> tuple[Data, nx.DiGraph, dict, dict, dict]:
    ...
    print(f"DataFrame now has {len(df):,} edges after adding compound edges.")
